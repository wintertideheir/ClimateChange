...
struct Mesh geodesic_icosahedron(int subdivisions) {
  ...
  // Alternating buffers to store intermediate subdivision data.
  ...
  // For arbitrarily adding to the next buffer.
  int vertices_to_length = 0;
  int faces_to_length = 0;

  // A map of two faces to their respective dynamically generated edge.
  int edge_map_length = 0;
  unsigned int edge_map[ICOSAHEDRON_EDGES(subdivisions)][3];

  // Add a new vertex to the next buffer.
  #define vertex(x, y, z) \
  ...
  // Add an old vertex to the next buffer.
  #define old_vertex(v) \
    vertex((*vertices_from)[v][0], \
           (*vertices_from)[v][1], \
           (*vertices_from)[v][2]); \

  // Add a new vertex whose direction is the same as a midpoint
  // and whose vector length is 1
  ...
  // Retrieve, and create if necessary, an edge from the edge map.
  #define edge(from, to, store) \
    store = -1; \
    for (int s = 0; s < edge_map_length; s++) { \
      if ((edge_map[s][0] == from && edge_map[s][1] == to)    || \
          (edge_map[s][0] == to   && edge_map[s][1] == from)) { \
        store = edge_map[s][2]; \
  ...
  // Perform the icosahedron subdivision.
  for (int i = 0; i < subdivisions; i++) {
    ...
    // Switch the buffer references and reset associated variables.
